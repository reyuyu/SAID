...
class MaskNetwork(nn.Module):
	def __init__(self, width, layers=1, heads=8, attn_mask=None):
		...
		self.attn_pool = AttentionPool(width)
		nn.init.normal_(self.attn_pool.attention.weight, std=width ** -0.5)
		nn.init.zeros_(self.attn_pool.attention.bias)
...
class CLIP(nn.Module):
	def __init__(self,
		...
		self.mask_net = MaskNetwork(transformer_width, layers=1, heads=transformer_heads)
	...
	def encode_text_with_checkpoint(self, text):
		...
		x = checkpoint.checkpoint(self.transformer, x)
		x = x.permute(1, 0, 2)  # LND -> NLD
		y = self.ln_final(x).type(self.dtype)

		# x.shape = [batch_size, n_ctx, transformer.width]
		# take features from the eot embedding (eot_token is the highest number in each sequence)
		x = y[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection

		return x, y

	def encode_text(self, text, return_full=False, return_pool=False):
		x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
		...
		x = y[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
		if return_pool:
			return y[torch.arange(x.shape[0]), text.argmax(dim=-1)]
		if return_full:
			return x, y
		return x
	...
	def compute_smart_image_feature(self, unnorm_image_features, full_text_embedding, soft=False):
		...
		image_features = unnorm_image_features * use_mask
		image_features = image_features / image_features.norm(dim=1, keepdim=True)
		return image_features, hard_mask
	...
	def forward(self, image, text, rank, soft=False):
		...
		#============================
		# same image, different mask, similarity directly
		#============================
		rep_use_mask_all_sidm = use_mask_all.repeat(bs, 1)
		rep_unnorm_image_features_long_sidm = unnorm_image_features_long.repeat_interleave(len(use_mask_all), dim=0)
		rep_image_features_sidm = rep_unnorm_image_features_long_sidm * rep_use_mask_all_sidm
		rep_image_features_sidm = rep_image_features_sidm / rep_image_features_sidm.norm(dim=1, keepdim=True)
		...
		sidm_sim = 100 * sidm_sim
		loss_sidm = F.cross_entropy(sidm_sim, targets)

		#============================
		# different image, same mask, similarity directly
		#============================
		unnorm_image_features_long_all = torch.cat(nn_dist.all_gather(unnorm_image_features_long), dim=0)
		ns = len(use_mask_all)
		rep_use_mask_dism = use_mask.repeat_interleave(ns, dim=0)
		rep_unnorm_image_features_long_all_dism = unnorm_image_features_long_all.repeat(bs, 1)
		rep_image_features_dism = rep_unnorm_image_features_long_all_dism * rep_use_mask_dism
		rep_image_features_dism = rep_image_features_dism / rep_image_features_dism.norm(dim=1, keepdim=True)
		...
		'num_min': torch.sum((hard_mask), dim=1).float().min().item(),
		'num_max': torch.sum((hard_mask), dim=1).float().max().item(),
		}
		return loss_dict
...
def build_model(state_dict: dict, load_from_clip: bool, args=None):
	...
	model = CLIP(
		...
		context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, load_from_clip, args=args,
	)
