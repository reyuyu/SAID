...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, args=None):
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip = False, args=args).to(device)
...
def load_from_clip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, args=None):
    ...
    model = build_model(state_dict or model.state_dict(), load_from_clip = True, args=args).to(device)
    ...
    for i, tokens in enumerate(all_tokens):
        if len(tokens) > context_length:
            if truncate:
                ...
                tokens[-1] = eot_token
                #print(f"Truncating input with {len(tokens)} tokens to {context_length} tokens")
            else:
