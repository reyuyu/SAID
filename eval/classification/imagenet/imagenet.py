...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i, (images, targets, paths) in pbar:
            ...
            for i in range(total_len):
                ...
                if pred[i].item() == label:
                    true_num += 1
                total_num += 1
            pbar.set_description(f"Top 1 accuracy: {true_num / total_num:.4f}")
            #save_to_file(logits, targets, paths)
    print(true_num / total_num)

if __name__ == "__main__":
    args = argparse.ArgumentParser(description='CLIP inference')
    args.add_argument('-d', '--data-dir', default='../../datasets/ILSVRC', type=str,
    ...
    args.add_argument('--checkpoint', type=str, default='')
    args.add_argument('--type', type=str, default='standard')
