"""SAID S0-TriMask v0.1 trainer -- ``objective = smartclip_trimask``.
...
text branch and the checkpoint payload.
...
so no extra ``world_size`` factor is applied anywhere. This is the reference S0 convention (the
baseline this experiment is compared against) and it is verified against a single-process run in
``tests/test_said_trimask.py``. The frozen shared init is loaded strictly, only the unused
``logit_scale`` is frozen and excluded from the optimizer, and the LR horizon stays at the full
...
from model.said_trimask import (ARM, LAMBDA_1, LAMBDA_2, LAMBDA_3, LAMBDA_SPARSE_I,  # noqa: E402
                                OBJECTIVE, PHASE, TriMaskTrainModule)
...
def trimask_train_step(ddp_model, batch, optimizer, mask_optimizer, device, amp_dtype,
                       amp_enabled=True, capture_grads=False):
    ...
    first cross-rank gather so a length mismatch is reported instead of aborting inside gloo.
    """
...
def main():
    parser = argparse.ArgumentParser(description='SAID S0-TriMask v0.1')
    parser.add_argument('--arm', default=ARM, choices=[ARM])
    parser.add_argument('--objective', default=OBJECTIVE, choices=[OBJECTIVE])
    ...
    config = {
        ...
        'lambda_sparse_i': args.lambda_sparse_i, 'lambda_sparse_t': 0.0, 'lambda_adv': 0.0,
        'sparse_definition': 'mean_d abs(mI_i,d) over the REAL matched pairs of this rank, '
                             'counted once (never tripled for the three paths)',
        'visual_mask': 'hard straight-through mask_net(C_S hidden) exactly as the S0 Said forward',
        'text_mask': 'independent MaskNetwork text_stem + gate projection; '
                     'mT = 0.1 + 0.9*sigmoid(gate); soft suppression, no threshold, no top-k',
        'text_mask_init': 'gate weight 0 and bias log(8) => mT = 0.9 everywhere => '
                          'Norm(t*mT) == Norm(t) at step 0',
    ...
    if args.resume:
        ...
        if payload.get('objective') != OBJECTIVE or payload.get('arm') != ARM:
            raise ValueError('resume objective/arm mismatch')
    ...
    if rank == 0:
        summary = {
            'arm': ARM, 'objective': OBJECTIVE, 'phase': PHASE, 'completed_steps': completed,
            ...
            'peak_memory_gb': torch.cuda.max_memory_allocated(device) / (1024 ** 3),
            'lambda_1': args.lambda_1, 'lambda_2': args.lambda_2, 'lambda_3': args.lambda_3,
            'lambda_sparse_i': args.lambda_sparse_i,
        }
