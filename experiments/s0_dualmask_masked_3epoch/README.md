# S0 Dual-Mask masked：500 → 1000 → 3 epoch 复现实验包
...
当前完成情况以 [status.json](evidence/status.json) 的采集时间为准。此包首次发布时已完成 1000 步与两项测评，3 epoch 续训仍在运行。不能把 3651 的计划当作已完成结果。
...
来源为本目录 `evidence/masked_formal500_report.json`、`evidence/step1000/coco_canonical_step1000.json`、`evidence/step1000/urban1k1000.json`。1000 步训练、严格导出、COCO、Urban 四阶段退出码均为 0。没有普通后缀监督的正式对照臂，不能用这些结果单独归因于新 mask。
...
只同步本目录不会改变训练代码。运行中的训练工作区须保持在固定 SHA，文档发布在独立工作区完成。实际脚本、配置和历史结果中的服务器绝对路径是出处信息；可移植命令见 `REPRODUCE.md`。
