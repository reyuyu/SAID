...
resume_ckpt="$run_dir/s0_dual_mask_suffix_masked_step000500.pt"
expected_sha=11af80b344c623b27b93069f9be526970c9c950c
trap 'code=$?; printf "%s\n" "$code" > "$run_dir/resume.exitcode"' EXIT
test ! -e "$run_dir/resume_started.txt" || exit 96
...
nvidia-smi --query-gpu=index,name,memory.total,memory.used,utilization.gpu --format=csv > "$run_dir/gpu_before_resume500.csv"
git rev-parse HEAD > "$run_dir/training_sha.txt"
...
date --iso-8601=seconds > "$run_dir/resume_started.txt"
set +e
...
run_exit=$?
printf '%s\n' "$run_exit" > "$run_dir/resume.exitcode"
printf '%s\n' "$(( $(date +%s) - train_started ))" > "$run_dir/resume_wall_seconds.txt"
date --iso-8601=seconds > "$run_dir/resume_finished.txt"
exit "$run_exit"
