...
def main():
    ...
    result={'experiment':'FineLIP-Prefix FP0','description':'依据FineLIP方法与公开技术说明独立实现的前缀监督基线',
            ...
            'validation':read(dest/'validation.json'),'NOT_RUN':['UPSTREAM_RUNTIME_PARITY','update1000 evaluation omitted at user request','full formal worker-resume experiment','six-epoch training'],
    ...
    text=['# FineLIP-Prefix FP0：两 epoch 实验报告','',result['description']+'。','',
          ...
          '保存 init/1000/epoch1/epoch2；按用户后续要求跳过1000步评估，仅评估init/epoch1/epoch2。严格导出317个学生 tensors，主评估只用原生 CLS/EOS。两次epoch尾部均实际累积2个microbatch、%d对样本，已flush。'%(tails[0]['group_samples']),'',
